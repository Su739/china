define(function(require){
  ...
  var parent = document.getElementById("area-items");
  ...
  EventUtil.addHandler(parent, "click",function(event){
    ...
    search(areaJson, target);
  });

  function search(items, elem)
  {
    var value = elem.getAttribute("title");
    
    if(isArray(items)){
      var b = items.forEach(function(item){
        if(item.value === value){
          show(item);
        }
        else{
  ...
  function show(items){
    ...
    var ul = document.getElementById("u2"); 
    var li = null;
